...
class CensusProvider extends HttpRequest {
    constructor () {
        // api api
        super()
    }
    ...
    async getCensus () {
        ...
        this.setHeader({})
        // example path http://172.28.26.82:7001/this.get()
        const {data} = await this.get('cencus')
    ...
    async updateCensus(
        ...
        var bodyFormData = new FormData();
        bodyFormData.append('cencus_id',cencus_id_ref);
        bodyFormData.append('village_id',village_id_ref);
        ...
        const {data} = await this.create('cencus/'+census_id,bodyFormData)
